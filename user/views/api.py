...
from ..permissions import IsYou ,IsSuperUser
...
class StudentProfileAPI(viewsets.ViewSet):
    ...
    def get_permissions(self):
        ...
        if self.action == 'retrieve':
            permission_classes = [IsYou or IsAuthenticated,]

        else:
            permission_classes = [IsAuthenticated]
        # return permission_classes
        return [permission() for permission in permission_classes]
        # return [OR(IsYou(),IsSuperUser())]
    ...
    def partial_update(self, request, pk=None):
        queryset = m.Student.objects.all()
        user = get_object_or_404(queryset, pk=pk)
        serializer = s.StudentInfo(instance=user, data=request.data,context={"request": request})
        if request.data['email'] != user.email and serializer.is_valid(raise_exception=True) :
            ...
            if request.accepted_renderer.format == 'json' or request.accepted_renderer.format == 'api':
                ...
                return Response(template_name='user/tests.html')
        serializer = s.StudentInfo(instance=user, data=request.data , context={"request": request},partial=True)
